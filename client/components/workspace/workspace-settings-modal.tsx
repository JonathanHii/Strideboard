  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm transition-all duration-300">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90dvh] sm:max-h-[90vh] transform transition-all animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 overflow-y-auto custom-scrollbar flex-1 [scrollbar-gutter:stable]">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <input
                      ...
                      /* CHANGED: Replaced flex-1 with w-full sm:flex-1 and added shrink-0 */
                      className="w-full sm:flex-1 px-4 h-11 shrink-0 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-medium text-gray-900"
                    />
                    <button
                ...
                <div className="flex items-center gap-3 p-4 bg-blue-50 text-blue-700 rounded-xl text-sm">
                  ...
                  <div>
                    ...
                    <span className="opacity-75 font-mono break-all">{workspace.id}</span>
                  </div>
                </div>
              </div>
            </section>
                ...
                {currentUser && (
                  <div className="flex items-center justify-between p-3 border border-indigo-200 bg-indigo-50/50 rounded-xl">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-8 h-8 rounded-full flex shrink-0 items-center justify-center font-bold text-xs ${getAvatarColor(currentUser.role)}`}>
                        You
                      </div>
                ...
                {sortedMembers.map((member) => {
                  ...
                  return (
                    <div key={member.id} className="flex items-center justify-between p-3 border border-gray-100 rounded-xl">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-8 h-8 rounded-full flex shrink-0 items-center justify-center font-bold text-xs ${getAvatarColor(member.role)}`}>
                      ...
                      {canEditRole && (
                        <button
                          ...
                          className="text-gray-400 hover:text-red-600 disabled:text-gray-300 transition-colors p-1 shrink-0"
                          title="Remove member"
                        >
              ...
              {amIOwner && (
                <div className="border border-red-100 bg-red-50 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-4 py-2 bg-white text-red-600 border border-red-200 hover:bg-red-50 hover:border-red-300 rounded-lg text-sm font-medium transition-all shadow-sm whitespace-nowrap justify-center"
                  >
                    Delete Workspace
                  </button>
                </div>
