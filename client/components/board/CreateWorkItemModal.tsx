...
export default function CreateWorkItemModal({
    ...
    // Members State - loaded once when modal opens
        ...
        if (isOpen && workspaceId) {
            const fetchMembers = async () => {
                ...
                try {
                    ...
                    setMembers(data);
                    setFilteredMembers(data); // Initially show all members
                } catch (err) {
    ...
    const handleCreateWorkItem = async () => {
        ...
        try {
            ...
            alert(
                error.message || "Failed to create work item. Please try again."
            );
        } finally {
            setIsCreating(false);
        }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm transition-all duration-300">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] transform transition-all animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-white shrink-0 z-20 rounded-t-2xl">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-0.5">
                            Add a new task, bug, or epic to your board.
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                {/* Added pb-32 to allow scrolling space for the dropdown at the bottom */}
                <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1 pb-54 [scrollbar-gutter:stable]">
                    {/* Title Input */}
                    <div>
                    ...
                    <div>
                        ...
                        {selectedAssignee ? (
                            <div className="flex items-center justify-between p-3 bg-indigo-5 border border-indigo-100 rounded-xl">
